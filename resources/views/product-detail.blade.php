<x-layout :linkWA="$linkWA"> {{-- Asumsi layout Anda menerima link WA --}}
    
    <x-slot:title>
        {{ $produk->nama }} {{-- Judul halaman dinamis --}}
    </x-slot:title>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">

            {{-- 1. Galeri Gambar Produk --}}
            <div class="w-full">
                {{-- Gambar Utama --}}
                <img src="{{ $produk->galeri_gambar[0] }}" alt="{{ $produk->nama }}" class="w-full h-auto md:h-[500px] object-cover rounded-lg shadow-lg">
                
                {{-- Thumbnail (jika ada lebih dari 1 gambar) --}}
                @if(count($produk->galeri_gambar) > 1)
                    <div class="grid grid-cols-4 gap-4 mt-4">
                        @foreach($produk->galeri_gambar as $gambar)
                            <img src="{{ $gambar }}" alt="thumbnail" class="h-24 w-full object-cover rounded-md cursor-pointer border-2 border-transparent hover:border-primary">
                        @endforeach
                    </div>
                @endif
            </div>

            {{-- 2. Info Produk & Tombol Aksi --}}
            <div class="w-full">
                ...
                {{-- Tombol "Pesan via WhatsApp" Interaktif --}}
                @php
                    // Membuat pesan WA spesifik untuk produk ini
                    $pesanProduk = "Halo, saya tertarik dengan produk '{$produk->nama}' (Harga: {$produk->harga}). Apakah stoknya masih ada?";
                    $nomorWA = '6281234567890'; // Ganti nomor ini
                    $linkWAPesanan = "https://wa.me/{$nomorWA}?text=" . urlencode($pesanProduk);
                @endphp
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    {{-- 
                      Menggunakan {!! !!} agar tag HTML (seperti <p>) di deskripsi bisa di-render.
                      Pastikan data ini aman (bukan dari input user) untuk menghindari XSS.
                    --}}
