<x-layout :linkWA="$linkWA">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section class="mb-16">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-8 mt-8">
                @foreach($produkUnggulan as $produk)
                    <div class="bg-white dark:bg-dark rounded-lg md:rounded-xl shadow-md overflow-hidden border border-gray-100 dark:border-gray-700 relative
                        ...
                        @if($produk['diskon'])
                            <span
                                class="absolute top-4 left-4 px-3 py-1 rounded-full text-sm font-semibold bg-red-500 text-white z-10">
                                Diskon {{ $produk['diskon']}}%
                            </span>
                        @elseif($produk['unggulan'])
                            <span
                                class="absolute top-4 left-4 px-3 py-1 rounded-full text-sm font-semibold bg-accent text-dark z-10">
                                Best Seller
                            </span>
                        @endif
                        ...
                        <div class="relative group">
                            <img src="{{ $produk['gambar'] }}" alt="{{ $produk['nama'] }}"
                                class="h-48 md:h-64 w-full object-cover transition-transform duration-500 group-hover:scale-110">
                        </div>
                        ...
                        <div class="p-3 md:p-5">
                            ...
                            <p class="text-primary font-bold mt-2 text-md md:text-lg dark:text-primary-light">
                                Rp.
                                {{ number_format(intval(str_replace(['Rp', '.', ','], '', $produk['harga'])), 2, ',', '.') }}
                            </p>
        ...
        <section class="mt-20">
            ...
            <!-- Search & Filter -->
            <form method="GET" class="mt-8 mb-8 p-6 bg-white dark:bg-dark rounded-xl shadow-md border border-gray-100 dark:border-gray-700
                        flex flex-col md:flex-row gap-4">
                <div class="flex-grow">
                    <label for="search" class="block text-sm font-medium text-dark dark:text-light mb-1">Cari
                        Produk</label>
                    <input type="text" name="search" id="search" placeholder="Ketik nama produk..."
                        value="{{ request('search') }}" class="w-full dark:bg-dark-light rounded-lg border-gray-300 dark:border-gray-600 shadow-sm px-3 py-2 text-dark dark:text-white
                               focus:border-accent focus:ring-accent dark:focus:border-accent dark:focus:ring-accent">
                </div>
                <div class="md:w-1/4">
                    <label for="kategori"
                        class="block text-sm font-medium text-dark dark:text-light mb-1">Kategori</label>
                    <select name="kategori" id="kategori" class="w-full dark:bg-dark-light dark:text-white rounded-lg border-gray-300 dark:border-gray-600 shadow-sm px-3 py-2
                           focus:border-accent focus:ring-accent dark:focus:border-accent dark:focus:ring-accent">
